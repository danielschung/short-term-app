...
const validate = () => {
	...
	let inputs = container.find('input:required, select:required, input[type="checkbox"]').toArray();
	...
	let empty = container.find('input.invalid, select.invalid');
	if ( empty.length !== 0) {
		alert();
	} else {
		removeAlert();
