...
const validate = () => {
	...
	const validateRadio = () => {
		...
		const checkRepresentative = () => {
			let representativeYes = container.find('input[type=radio][name="Request.Applicant.IsLegallyAuthorized"][value=yes]');
			let representativeNo = container.find('input[type=radio][name="Request.Applicant.IsLegallyAuthorized"][value=no]');
...
const addSpouseSignature = () => {
	let codeBlock = '<div class="codeBlock"> <input type="text" placeholder="Signature of Spouse or Civil Union (ONLY if to be insured)" required id="spouse-agreement-signature" name="Request.Spouse.AgreementSignature" minlength="1" maxlength="128"> <input type="text" placeholder="Date of Sign" required id="spouse-agreement-signature-date" name="Request.Spouse.AgreementSignatureDate" class="date" onfocus="(this.placeholder=\'MM-DD-YYYY\')" onblur="(this.placeholder=\'Date of Sign\')" minlength="8" maxlength="10"/> </div>';
