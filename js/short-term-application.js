...
const numOfDependents = () => {
	...
	const matchBox = () => {
		...
		for (let i = 0; i < amount; i++) {
			...
			let codeSignatureBlock2 = `<div class="codeBlock"> <input type="text" placeholder="Signature of Dependent ${depNumUI}(ONLY if to be insured and 18 or older)" id="dependent${depID}-signature" class="signature dependent-signature" name="Request.Dependents${depNum}.AgreementSignature" minlength="2" maxlength="128" required/> <input type="text" placeholder="Date of Sign" id="dependent${depID}-signature-date" name="Request.Dependents${depNum}.AgreementSignatureDate" class="date dependent-signature-date" onfocus="(this.placeholder='MM/DD/YYYY')" onblur="(this.placeholder='Date of Sign')" onkeyup="automask();" minlength="8" maxlength="10" required/> </div>`;
