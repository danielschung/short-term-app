...
var calculateAge = function calculateAge() {
  ...
  var setValue = ageBox.val(age); // console.log(ageBox.val());
}; //fields missing
...
var checkAgeSignature = function checkAgeSignature() {
  ...
  for (var i = 0; i < dependentBlocks.length; i++) {
    if (dependentBlocks[i].value >= 18) {
      var optional = signatureArray[i];

      for (var x = 0; x < signatureArray.length; x++) {
        $(optional).children('input').addClass('optional');
        console.log(dependentBlocks[i].value);
      }
    } else {
      var _optional = signatureArray[i];
      $(_optional).children('input').removeClass('optional');
